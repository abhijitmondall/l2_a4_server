...
const addMedicine = async (sellerId, payload) => {
...
const updateMedicine = async (sellerId, medicineId, payload) => {
...
const getSellerOrders = async (sellerId, status) => {
    ...
    return sellerOrders.map((order) => {
        ...
        const otherSellerItems = order.items.filter((item) => item.sellerId !== sellerId);
        // Calculate payouts
        const sellerPayout = myItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
        const otherPayout = otherSellerItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
        return {
            ...order,
            myItems,
            otherSellerItems,
